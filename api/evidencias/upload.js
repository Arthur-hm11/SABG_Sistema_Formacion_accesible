...
function isWithinJuly2026TemporaryWindow(date = new Date()) {
...
function isAllowedEvidencePeriod(month, year, date = new Date()) {
  ...
  if (normalizedMonth === normalizeText("Julio") && String(year) === "2026") {
    return isWithinJuly2026TemporaryWindow(date);
  }

  const currentPeriod = getCurrentEvidencePeriod();
  return normalizedMonth === normalizeText(currentPeriod.month) && String(year) === String(currentPeriod.year);
}
...
function getEvidencePeriodValidationMessage(month, year, date = new Date()) {
  ...
  if (
    normalizeText(month) === normalizeText("Julio") &&
    String(year) === "2026" &&
    !isWithinJuly2026TemporaryWindow(date)
  ) {
    return "El periodo extraordinario para cargar evidencia de julio 2026 concluyó el 10 de julio de 2026.";
  }
